...
class GameRepository(GameRepositoryPort):
    ...
    def get_games(
        ...
        return [
            Game(id=game.id, word=game.word, start_date=game.start_date)
            for game in raw_games
        ]

    def add_game(self, word: str) -> Game:
        with self._database.get_session() as session:
            new_game = GameModel(word=word, start_date=datetime.datetime.now())
            session.add(new_game)
            session.commit()
            session.refresh(new_game)

        return Game(id=new_game.id, word=new_game.word, start_date=new_game.start_date)
